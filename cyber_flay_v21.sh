...
# ========= OSINT FUNCTIONS =========
whois_lookup() { need whois; read -p "Masukkan domain: " d; whois "$d"; pause; }
geoip_lookup() { need curl jq; read -p "Masukkan IP/domain: " ip; curl -s -A "$ua" "https://ipinfo.io/${ip}" | jq .; pause; }
extract_links() { need curl; read -p "Masukkan URL (https://...): " u; curl -s -A "$ua" "$u" | grep -Eo '(http|https)://[^"'"'<> ]+' | sort -u; pause; }
http_headers() { need curl; read -p "Masukkan URL (https://...): " u; curl -s -I -A "$ua" "$u"; pause; }
ssl_info() { need openssl; read -p "Masukkan domain: " d; echo | openssl s_client -servername "$d" -connect "$d:443" 2>/dev/null | openssl x509 -noout -issuer -subject -dates -fingerprint -sha256; pause; }
security_headers() { need curl; read -p "Masukkan URL (https://...): " u; curl -s -I -A "$ua" "$u" | grep -Ei 'strict-transport|x-frame|x-xss|content-security|referrer-policy|permissions-policy' || echo "Header keamanan tidak ditemukan"; pause; }
threatfox_lookup() { need curl jq; read -p "Masukkan IOC (IP/Domain/Hash): " ioc; curl -s -A "$ua" "https://threatfox-api.abuse.ch/api/v1/" -H "Content-Type: application/json" -d "{\"query\":\"search_ioc\",\"search_term\":\"$ioc\"}" | jq .; pause; }
dns_lookup() { need nslookup; read -p "Masukkan domain: " d; nslookup "$d"; pause; }
reverse_dns() { need dig; read -p "Masukkan IP: " ip; dig -x "$ip" +short; pause; }
subdomain_finder() { need curl jq; read -p "Masukkan domain: " d; curl -s -A "$ua" "https://crt.sh/?q=%25.$d&output=json" | jq -r '.[].name_value' | sed 's/\*\.//g' | sort -u; pause; }
email_breach() { need curl jq; read -p "Masukkan email: " e; curl -s -A "$ua" "https://haveibeenpwned.com/unifiedsearch/$e" | jq .; pause; }
port_scan() { need nmap; read -p "Masukkan target (IP/host): " t; nmap -F "$t"; pause; }
asn_lookup() { need whois; read -p "Masukkan IP: " ip; whois "$ip" | grep -Ei 'origin|route|descr|AS'; pause; }
crawl_website() { need wget grep; read -p "Masukkan URL (https://...): " u; wget -r --spider -e robots=off -nd -nH "$u" 2>&1 | grep '^--' | awk '{print $3}' | sed 's/(.*)//' | sort -u; pause; }
...
username_check() {
  ...
  echo "================================" >> "$out"
  sites=( facebook.com instagram.com twitter.com github.com reddit.com tiktok.com linkedin.com youtube.com pinterest.com medium.com dev.to tumblr.com vimeo.com dailymotion.com soundcloud.com mixcloud.com last.fm stackoverflow.com quora.com about.me producthunt.com dribbble.com behance.net gitlab.com bitbucket.org sourceforge.net wordpress.com blogger.com weebly.com ok.ru vk.com flickr.com myspace.com 500px.com patreon.com kickstarter.com indiegogo.com slack.com discord.com telegram.me snapchat.com mastodon.social goodreads.com wattpad.com tripadvisor.com couchsurfing.com airbnb.com etsy.com depop.com steamcommunity.com twitch.tv strava.com deviantart.com keybase.io tradingview.com hackaday.io gumroad.com )
...
# ========= Browser Launcher =========
menu_browser() {
  need tor torsocks w3m
  banner
  echo -e "${CYN}--- Browser Launcher ---${RST}"
  echo "[1] Buka DuckDuckGo (clear web)"
  echo "[2] Masukkan URL sendiri (clearnet)"
  echo "[3] Masukkan URL .onion (via Tor)"
  echo "[0] Kembali"
  read -p "Pilih: " b
  case "$b" in
    1) w3m https://duckduckgo.com ;;
    2) read -p "Masukkan URL: " url; w3m "$url" ;;
    3)
       echo -e "${YLW}[+] Menjalankan Tor...${RST}"
       tor > /dev/null 2>&1 &
       sleep 5
       read -p "Masukkan URL .onion: " onion
       torsocks w3m "$onion"
       ;;
    0) return ;;
...
# Ensure base tools
need curl jq figlet openssl whois nmap wget grep coreutils bc tor torsocks w3m
